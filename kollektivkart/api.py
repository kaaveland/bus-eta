...
def set_headers(response: Response) -> Response:
    ...
    response.headers["Expires"] = (datetime.now() + timedelta(seconds=3600)).strftime('%a, %d %b %Y %H:%M:%S GMT')
...
@app.route(
    "/hot-spots/<int:year>/<int:month>/<int:hour>"
)
def hot_spots(
...
@app.route(
    "/leg-stats/<int:year>/<int:month>/<int:hour>/<datasource>"
)
def leg_stats(
    year: int, month: int, hour: int, datasource: str
) -> Response:
    partition = date(year, month, 1)
...
def lines(datasource: str) -> Response:
    data = (
        ...
        for label, ref in
        queries.lines_for_datasource(g.db, datasource).items()
    )
...
@app.route(
    "/rush-intensity-rank/<int:year>/<int:month>"
)
def rush_intensity_rank(
    year: int, month: int
) -> Response:
    partition = date(year, month, 1)
    return to_json(
        queries.most_rush_intensity(g.db, partition, request.args.get("data_source"), limit=100)
    )